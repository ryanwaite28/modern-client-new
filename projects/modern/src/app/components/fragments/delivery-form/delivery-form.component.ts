...
export class DeliveryFormComponent implements AfterViewInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.chargeFeeData = this.stripeService.add_on_stripe_processing_fee(this.deliveryForm.value.payout, this.is_subscription_active);

    this.deliveryForm.get('payout')?.valueChanges.subscribe((value) => {
      if (value) {
        this.chargeFeeData = this.stripeService.add_on_stripe_processing_fee(value, this.is_subscription_active);
      }
    });
  }
