...
export class DeliveryService {
  ...
  get_delivery_by_id<T = any>(delivery_id: number) {
    return this.clientService.sendRequest<T>(`/deliveries/${delivery_id}`, `GET`);
  }

  create_delivery<T = any>(data: FormData) {
    return this.clientService.sendRequest<T>(`/deliveries`, `POST`, data);
  }

  update_delivery<T = any>(data: FormData, delivery_id: number) {
    return this.clientService.sendRequest<T>(`/deliveries/${delivery_id}`, `PUT`, data);
  }

  delete_delivery<T = any>(delivery_id: number) {
    return this.clientService.sendRequest<T>(`/deliveries/${delivery_id}`, `DELETE`);
  }
  ...
  findAvailableDeliveryByFromCityAndState<T = any>(city: string, state: string) {
    return this.clientService.sendRequest<T>(`/deliveries/find-available-from/city/${city}/state/${state}`, `GET`);
  }

  findAvailableDeliveryByToCityAndState<T = any>(city: string, state: string) {
    return this.clientService.sendRequest<T>(`/deliveries/find-available-to/city/${city}/state/${state}`, `GET`);
  }

  findAvailableDelivery<T = any>(data: any) {
    return this.clientService.sendRequest<T>(`/deliveries/find-available`, `POST`, data);
  }
  ...
  searchDeliveries(data: any) {
    return this.clientService.sendRequest<IDelivery[]>(`/deliveries/search`, `POST`, data);
  }
  
  sendDeliveryMessage<T = any>(data: any) {
    return this.clientService.sendRequest<T>(`/deliveries/${data.delivery_id}/message`, `POST`, data);
  }
  
  payCarrier<T = any>(you_id: number, delivery_id: number) {
    return this.clientService.sendRequest<T>(`/deliveries/${delivery_id}/pay-carrier`, `POST`);
  }

  createPaymentIntent<T = any>(delivery_id: number) {
    return this.clientService.sendRequest<T>(`/deliveries/${delivery_id}/create-payment-intent`, `POST`);
  }

  browseRecent(delivery_id?: number) {
    const endpoint = delivery_id
      ? `/deliveries/browse-recent/${delivery_id}`
      : `/deliveries/browse-recent`;
  ...
  browseMap(params: {
    ...
    const endpoint = `/deliveries/browse-map/swlat/${southWest.lat}/swlng/${southWest.lng}/nelat/${northEast.lat}/nelng/${northEast.lng}`;
    return this.clientService.sendRequest<IDelivery[]>(endpoint, `POST`, null);
  }
  ...
  request_carrier_location<T = any>(delivery_id: number) {
    return this.clientService.sendRequest<T>(`/deliveries/${delivery_id}/request-carrier-location`, `POST`);
  }

  accept_request_carrier_location<T = any>(delivery_id: number) {
    return this.clientService.sendRequest<T>(`/deliveries/${delivery_id}/accept-request-carrier-location`, `POST`);
  }

  decline_request_carrier_location<T = any>(delivery_id: number) {
    return this.clientService.sendRequest<T>(`/deliveries/${delivery_id}/decline-request-carrier-location`, `POST`);
  }

  carrier_share_location<T = any>(delivery_id: number) {
    return this.clientService.sendRequest<T>(`/deliveries/${delivery_id}/carrier-share-location`, `POST`);
  }

  carrier_unshare_location<T = any>(delivery_id: number) {
    return this.clientService.sendRequest<T>(`/deliveries/${delivery_id}/carrier-unshare-location`, `POST`);
  }
  ...
  carrier_update_location<T = any>(params: {
    ...
    return this.clientService.sendRequest<T>(`/deliveries/${params.delivery_id}/carrier-update-location`, `POST`, params);
  }
}
